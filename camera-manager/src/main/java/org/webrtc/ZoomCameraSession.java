...
public class ZoomCameraSession implements CameraSession {
    ...
    protected ZoomCameraSession.SessionState state;
    ...
    public boolean isCameraActive() {
        if (isCameraReleased() || state.equals(ZoomCameraSession.SessionState.STOPPED)) {
    ...
    public static void create(CreateSessionCallback callback, Events events, boolean captureToTexture, Context applicationContext, SurfaceTextureHelper surfaceTextureHelper, int cameraId, int width, int height, int framerate) {
        ...
        if (camera == null) {
            ...
            callback.onDone(new ZoomCameraSession(events, captureToTexture, applicationContext, surfaceTextureHelper, cameraId, camera, info, captureFormat, constructionTimeNs));
        }
    }
    ...
    protected ZoomCameraSession(Events events, boolean captureToTexture, Context applicationContext, SurfaceTextureHelper surfaceTextureHelper, int cameraId, Camera camera, Camera.CameraInfo info, CameraEnumerationAndroid.CaptureFormat captureFormat, long constructionTimeNs) {
    ...
    public void stop() {
        ...
        if (this.state != ZoomCameraSession.SessionState.STOPPED) {
    ...
    private void startCapturing() {
        ...
        this.checkIsOnCameraThread();
        this.state = ZoomCameraSession.SessionState.RUNNING;
        this.camera.setErrorCallback(new Camera.ErrorCallback() {
            public void onError(int error, Camera camera) {
                ...
                ZoomCameraSession.this.stopInternal();
                if (error == CAMERA_DISCONNECTED_ERROR) {
                    ZoomCameraSession.this.events.onCameraDisconnected(ZoomCameraSession.this);
                } else {
                    ZoomCameraSession.this.events.onCameraError(ZoomCameraSession.this, errorMessage);
                }
    ...
    private void stopInternal() {
        ...
        if (this.state == ZoomCameraSession.SessionState.STOPPED) {
            ...
            this.state = ZoomCameraSession.SessionState.STOPPED;
    ...
    private void listenForTextureFrames() {
        this.surfaceTextureHelper.startListening((frame) -> {
            this.checkIsOnCameraThread();
            if (this.state != ZoomCameraSession.SessionState.RUNNING) {
    ...
    private void listenForBytebufferFrames() {
        this.camera.setPreviewCallbackWithBuffer(new Camera.PreviewCallback() {
            public void onPreviewFrame(byte[] data, Camera callbackCamera) {
                ZoomCameraSession.this.checkIsOnCameraThread();
                if (callbackCamera != ZoomCameraSession.this.camera) {
                    Logging.e(TAG, CALLBACK_FROM_A_DIFFERENT_CAMERA_THIS_SHOULD_NEVER_HAPPEN);
                } else if (ZoomCameraSession.this.state != ZoomCameraSession.SessionState.RUNNING) {
                    Logging.d(TAG, BYTEBUFFER_FRAME_CAPTURED_BUT_CAMERA_IS_NO_LONGER_RUNNING);
                } else {
                    long captureTimeNs = TimeUnit.MILLISECONDS.toNanos(SystemClock.elapsedRealtime());
                    if (!ZoomCameraSession.this.firstFrameReported) {
                        int startTimeMs = (int)TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - ZoomCameraSession.this.constructionTimeNs);
                        ZoomCameraSession.camera1StartTimeMsHistogram.addSample(startTimeMs);
                        ZoomCameraSession.this.firstFrameReported = true;
                    }

                    VideoFrame.Buffer frameBuffer = new NV21Buffer(data, ZoomCameraSession.this.captureFormat.width, ZoomCameraSession.this.captureFormat.height, () -> {
                        ZoomCameraSession.this.cameraThreadHandler.post(() -> {
                            if (ZoomCameraSession.this.state == ZoomCameraSession.SessionState.RUNNING) {
                                ZoomCameraSession.this.camera.addCallbackBuffer(data);
                            }

                        });
                    });
                    VideoFrame frame = new VideoFrame(frameBuffer, ZoomCameraSession.this.getFrameOrientation(), captureTimeNs);
                    ZoomCameraSession.this.events.onFrameCaptured(ZoomCameraSession.this, frame);
                    frame.release();
                }
            }
        });
